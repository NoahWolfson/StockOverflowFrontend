...
const UserMessagesComponent: React.FC<userMessageProps> = (props) =>{
    return(
        <div className="UserMessagesComponent">
                {props.messages.map((message) => (<Stub Date_Created={message.Date_Created} RepliedTo={message.RepliedTo} Likes={message.Likes} Dislikes={message.Dislikes} Text={message.Text} key={message._id} IsQuestion={message.IsQuestion} _id={message._id} ></Stub>))}
        </div>
    )
}
